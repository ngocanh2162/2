...
class NLP2(object):
    ...
    def duplicateHandling(self):
        f = self.list1
        for i in range(len(f)+1):
            for line in f[i]:
                line = NLP(text=line).split_words()
...
l.append([['công_ty'], ['nhà_máy'], ['bến_xe'], ['khu_đô_thị'], ['khách_sạn'], ['nhà_nghỉ'], ['chung_cư'], ['lăng'], ['tòa'], ['tòa_nhà'], ['ga'], ['chợ'], ['cầu'], ['ngân_hàng'], ['bệnh_viện'], ['nghĩa_trang']])
...
for i in range(len(h)):
    for x in h[i]:
        k.append(x)
wordDict = dict.fromkeys(set(k), 0)
for word in k:
    wordDict[word]+=1
print(wordDict)
writeListToTextFile(k, open(dictionary_files[1], 'a',encoding="utf8"))
